...
const Profile = (props) => {
  ...
  const rs = (n) => Math.round((width / 375) * n); // responsive scale based on iPhone 11 width
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: rs(40) }}>
        ...
        <View style={[styles.header, { paddingVertical: rs(30) }]}>
          ...
          <Text style={[styles.headerTitle, { fontSize: rs(18) }]}>
            Profile
          </Text>
        </View>
        ...
        <View style={{ alignItems: "center", marginTop: rs(24) }}>
          ...
          >
            <Text
              style={{ color: "#FFFFFF", fontSize: rs(24), fontWeight: "700" }}
            >
              {user.firstName}
            </Text>
          </View>
          ...
          <Text
            style={{ color: "#A5A8B6", fontSize: rs(14), marginTop: rs(6) }}
          >
            @{user.username}
          </Text>
        </View>
        ...
        <View style={{ paddingHorizontal: rs(20), marginTop: rs(20) }}>
          ...
          >
            <Text style={[styles.cardLabel, { fontSize: rs(12) }]}>
              Username
            </Text>
            <View style={styles.cardRow}>
              <Text style={[styles.cardValue, { fontSize: rs(16) }]}>
                {user.username}
              </Text>
              <Ionicons
          ...
          <View
            style={[styles.card, { padding: rs(14), borderRadius: rs(14) }]}
          >
            <Text style={[styles.cardLabel, { fontSize: rs(12) }]}>
              Membership
            </Text>
        ...
        <View style={{ paddingHorizontal: rs(20), marginTop: rs(18) }}>
          {[
            ...
            <TouchableOpacity
              key={idx}
              style={[styles.listItem, { paddingVertical: rs(14) }]}
              activeOpacity={0.8}
            >
              <View
                style={[
                  styles.listIconWrap,
                  { width: rs(36), height: rs(36), borderRadius: rs(8) },
                ]}
              >
                <Ionicons name={item.icon} size={rs(18)} color="#C8B6FF" />
              </View>
              <Text style={[styles.listLabel, { fontSize: rs(15) }]}>
                {item.label}
              </Text>
